...
function safeSession(row: any) { return { id: row.id, email: row.email, source_channel: row.source_channel, run_count: Number(row.run_count || 0), max_runs: Number(row.max_runs || DEFAULT_MAX_RUNS), remaining_runs: Math.max(0, Number(row.max_runs || DEFAULT_MAX_RUNS) - Number(row.run_count || 0)), status: row.status, created_at: row.created_at, total_active_seconds: Number(row.total_active_seconds || 0) }; }
...
function formPayload(raw: any) {
  ...
  const rawTimeToCommit = Number(raw?.time_to_commit_seconds); const timeToCommitSeconds = Number.isFinite(rawTimeToCommit) ? Math.max(0, Math.min(86400, Math.round(rawTimeToCommit))) : null;
  return { title: asString(raw?.title, 180) || "Live incident", service_name: asString(raw?.service_name, 100) || "unknown-service", severity, symptom_text: symptomText, impact, consequence_type: consequenceType, decision_deadline: asIsoDate(raw?.decision_deadline), fingerprint: asStringArray(raw?.fingerprint, 24, 100), source_type: asString(raw?.source_type, 40) || "human", source_name: asString(raw?.source_name, 120), selected_branch: selectedBranch, supporting_evidence: supportingEvidence, counterevidence: asStringArray(raw?.counterevidence, 12), reasoning_summary: reasoningSummary, alternative_branches: asStringArray(raw?.alternative_branches, 12), unknowns: asStringArray(raw?.unknowns, 12), confidence: Math.round(confidence), abort_conditions: abortConditions, first_action: firstAction, time_to_commit_seconds: timeToCommitSeconds };
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "record_active_time") {
      const seconds = Math.max(0, Math.min(600, Math.round(Number(payload?.seconds)) || 0));
      if (seconds > 0) { const inc = await admin.rpc("add_session_active_seconds", { p_validation_session_id: session.id, p_seconds: seconds }); if (inc.error) throw inc.error; }
      const refreshed = await admin.from("validation_sessions").select("*").eq("id", session.id).maybeSingle();
      return json(req, { ok: true, session: refreshed.data ? safeSession(refreshed.data) : safeSession(session) });
    }

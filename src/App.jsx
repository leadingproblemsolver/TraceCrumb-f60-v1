...
function LandingPage({ onStart, onResume, hasSession, error }) {
  ...
  return <main className="landing-shell"><nav className="landing-nav"><div className="brand-mark"><span className="brand-dot" /><span>{BRANCH.product}</span><small>{BRANCH.productDescriptor}</small></div><div className="row"><ThemeToggle />{hasSession ? <button className="secondary glow-cta" onClick={onResume}>Resume validation</button> : null}</div></nav><section className="landing-hero"><div className="landing-hero-copy"><div className="kicker">{BRANCH.kicker}</div><p className="landing-tagline">{BRANCH.tagline}</p><h1>{BRANCH.headline}</h1><p className="landing-copy">{BRANCH.subheadline}</p><div className="landing-actions"><button className="glow-cta" type="button" onClick={onStart}>{hasSession ? 'Continue your 10-run validation' : 'Start your 10-run validation'}</button><a className="button-link secondary" href="?demo=1&source_channel=landing" onClick={() => logEvent('demo_opened')}>See a worked example</a></div><p className="field-help">One email, no password, no magic link. Your 10-run validation workspace is bound to this browser.</p>{error ? <p className="loss" role="alert">{error}</p> : null}<TrustRow /><ProcessStrip active={1} /></div><AuditPreview /></section><section className="landing-grid"><div className="card"><span className="kicker">Problem</span><h3>{BRANCH.landing.problemTitle}</h3><p>{BRANCH.landing.problemBody}</p></div><div className="card"><span className="kicker">Mechanism</span><h3>{BRANCH.landing.solutionTitle}</h3><p>{BRANCH.landing.solutionBody}</p></div><div className="card"><span className="kicker">Return</span><h3>{BRANCH.landing.proofTitle}</h3><p>{BRANCH.landing.proofBody}</p></div></section></main>;
}
...
function DecisionRecord({ record, onPostReviewAction, onOutcome, saving }) {
  ...
  {onPostReviewAction && !record.postReviewAction ? (() => { const ownerRequired = action === 'revise' || action === 'escalate'; return <section className="card"><span className="kicker">Mandatory next move</span><h2>Choose explicitly—nothing is preselected</h2><div className="form-grid"><label>Choice<select required value={action} onChange={(e) => setAction(e.target.value)}><option value="">Select…</option><option value="proceed">Proceed unchanged</option><option value="revise">Run the check or revise the theory</option><option value="escalate">Escalate for another review</option><option value="stop">Stop this action</option></select></label><label>Exact next move<input required value={finalBranch} onChange={(e) => setFinalBranch(e.target.value)} /></label><div className="two-col"><label>Owner{ownerRequired ? ' (required for revise/escalate)' : ''}<input required={ownerRequired} value={owner} onChange={(e) => setOwner(e.target.value)} /></label><label>Due at{ownerRequired ? ' (required for revise/escalate)' : ''}<input required={ownerRequired} type="datetime-local" value={dueAt} onChange={(e) => setDueAt(e.target.value)} /></label></div><label>Why this move?<textarea required value={reason} onChange={(e) => setReason(e.target.value)} /></label><button disabled={saving || !action || !reason.trim() || !finalBranch.trim() || (ownerRequired && (!owner.trim() || !dueAt))} onClick={() => onPostReviewAction({ action, final_branch: finalBranch, reason, owner, due_at: localDateTimeToIso(dueAt) })}>{saving ? 'Saving…' : 'Save explicit next move'}</button></div></section>; })() : null}
...
function FirstRunSignalModal({ decisionEventId, onDone }) {
  ...
  const complete = Boolean(triggerContext.trim() && decisionDelta.trim() && commercialThreshold.trim());
  function dismiss(kind) { window.localStorage.setItem(FIRST_RUN_SIGNAL_KEY, kind); logEvent(kind === 'saved' ? 'first_run_signal_saved' : 'first_run_signal_skipped'); onDone(); }
  async function submit(event) {
    event.preventDefault(); if (!complete) return; setBusy(true); setError('');
    try { await callApi('save_signup_signal', { decision_event_id: decisionEventId, trigger_context: triggerContext, decision_delta: decisionDelta, commercial_threshold: commercialThreshold, source_channel: getSourceChannel() }); dismiss('saved'); }
    catch (err) { setError(err.message || String(err)); } finally { setBusy(false); }
  }
  return <div className="modal-backdrop" role="dialog" aria-modal="true" onClick={() => dismiss('skipped')}><div className="card modal-card" onClick={(e) => e.stopPropagation()}><span className="kicker">Optional — asked once</span><h2>What just happened?</h2><p className="field-help">Your checkpoint is already saved. These three questions are optional and help us learn whether TraceCrumb is worth building further.</p><form className="form-grid" onSubmit={submit}><label>What happened today that made you try TraceCrumb now?<textarea maxLength="1200" value={triggerContext} onChange={(e) => setTriggerContext(e.target.value)} /></label><label>What changed or became sharper in your next move—and which part caused it?<textarea maxLength="1200" value={decisionDelta} onChange={(e) => setDecisionDelta(e.target.value)} /></label><label>What do you use or spend today instead, and what result across your next 10 decisions would make TraceCrumb worth buying?<textarea maxLength="1600" value={commercialThreshold} onChange={(e) => setCommercialThreshold(e.target.value)} /></label>{error ? <p className="loss" role="alert">{error}</p> : null}<div className="row"><button disabled={busy || !complete}>{busy ? 'Saving…' : 'Save answers'}</button><button className="secondary" type="button" onClick={() => dismiss('skipped')}>Skip</button></div></form></div></div>;
}
...
export default function App() {
  ...
  useEffect(() => { (async () => { if (!isSupabaseConfigured) { setLoading(false); return; } try { const data = await callApi('session_status'); if (data.session) { setValidationSession(data.session); setScreen('app'); } } catch (_) {} finally { setLoading(false); } })(); }, []);
